...
public class Booth_Past_eventAdapter extends RecyclerView.Adapter<Booth_Past_eventAdapter.MyViewHolder_procedBene> {
    Context context;
    ...
    public Booth_Past_eventAdapter(Context context, OnItemClickListener.OnClickCallback onClickCallback_Benefi, ArrayList<Row> booth_In_Past_Event_List) {
    ...
    public MyViewHolder_procedBene onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        if (appPreferences.getCulturemode().equals("1")) {
            ...
            v = LayoutInflater.from(parent.getContext()).inflate(R.layout.bootpast_sub_list, parent, false);

        } else {
            ((Activity) context).getWindow().getDecorView().setLayoutDirection(View.LAYOUT_DIRECTION_RTL);
            v = LayoutInflater.from(parent.getContext()).inflate(R.layout.bootpast_sub_list_ar, parent, false);
    ...
    public void onBindViewHolder(@NonNull MyViewHolder_procedBene holder, int position) {
        ...
        date.setText(booth_In_Past_Event_List.get(position).getEventName() + ",\n" +booth_In_Past_Event_List.get(position).getCommunityName() + " " + booth_In_Past_Event_List.get(position).getDate());
        proceed.setOnClickListener((View.OnClickListener) view -> {
