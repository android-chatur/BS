...
public class Upcoming_eventAdapter extends RecyclerView.Adapter<Upcoming_eventAdapter.MyViewHolder_procedBene> {
    Context context;
    ...
    public Upcoming_eventAdapter(Context context, OnItemClickListener.OnClickCallback onClickCallback_Benefi, ArrayList<Row> upComming_list) {
    ...
    public MyViewHolder_procedBene onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        if (appPreferences.getCulturemode().equals("1")) {
            ...
            v = LayoutInflater.from(parent.getContext()).inflate(R.layout.upcomin_sub_list, parent, false);

        } else {
            ((Activity) context).getWindow().getDecorView().setLayoutDirection(View.LAYOUT_DIRECTION_RTL);
            v = LayoutInflater.from(parent.getContext()).inflate(R.layout.upcomin_sub_list_ar, parent, false);
    ...
    public void onBindViewHolder(@NonNull MyViewHolder_procedBene holder, int position) {
        ...
        date.setText(upComming_list.get(position).getEventName() + ",\n" +upComming_list.get(position).getCommunityName() + " " + upComming_list.get(position).getDate());
